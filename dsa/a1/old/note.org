...
**** DONE void add_edge (const int&, const int&, const T&)
     CLOSED: [2020-05-17 Sun 18:48]
...
**** DONE void remove_edge (int&, int&)
     CLOSED: [2020-05-17 Sun 18:48]
     - Removes the edge between the two vertices, if it exists.

**** DONE size_t out_degree (int&) {return this->size;} const;
     CLOSED: [2020-05-17 Sun 18:49]
     - Returns the number of edges leaving a vertex.
**** DONE size_t->in_degree (int&) {return this->size;}
     CLOSED: [2020-05-17 Sun 18:49]
     - Returns number of edges coming in to a vertex.
**** DONE size_t degree(const int&) {return this->size; const;
     CLOSED: [2020-05-17 Sun 18:49]
...
**** DONE vector<vertex<T>> get_neighbours(int&) {vertex<T> v; return v;}
     CLOSED: [2020-05-17 Sun 18:49]
...
**** DONE vector<vertex<T>> get_second_order_neighbors(const int&) {vertex<T> v; return v;}
     CLOSED: [2020-05-17 Sun 18:49]
...
**** DONE bool reachable(const int&, const int&) const;
     CLOSED: [2020-05-17 Sun 18:49]
...
**** DONE vector<vertex<T>> breadth_first(const int&) {vertex<T> v; return v;};
     CLOSED: [2020-05-17 Sun 18:49]
     - Returns the vertices of the graph in the order they are visisted in by a breadth-first traversal
     - starting at the given vertex.

**** DONE directed_graph<T> out_tree(const int&);
     CLOSED: [2020-05-18 Mon 05:38]
