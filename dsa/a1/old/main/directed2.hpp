...
public:
	...
	// bool adjacent(const int&, const int&) {return false;} const; //Returns true if the first vertex is adjacent to the second, false otherwise.
	...
	// size_t in_degree(const int&) {return this->size;} const; //Returns number of edges coming in to a vertex.
	// size_t out_degree(const int&) {return this->size;} const; //Returns the number of edges leaving a vertex.
	// size_t degree(const int&) {return this->size; const; //Returns the degree of the vertex (both in edges and out edges).

	// size_t num_vertices() {return this->size;} const; //Returns the total number of vertices in the graph.
	// size_t num_edges() {return this->size;} const; //Returns the total number of edges in the graph.

	// vector<vertex<T>> get_vertices() {vertex<T> v; return v;} ; //Returns a vector containing all the vertices.
	// vector<vertex<T>> get_neighbours(const int&) {vertex<T> v; return v;}; //Returns a vector containing all the vertices reachable from the given vertex. The vertex is not considered a neighbour of itself.
	// vector<vertex<T>> get_second_order_neighbors(const int&) {vertex<T> v; return v;}; // Returns a vector containing all the second_order_neighbours (i.e., neighbours of neighbours) of the given vertex.
	...
	// vector<vertex<T>> depth_first(const int&) {vertex<T> v; return v;}; //Returns the vertices of the graph in the order they are visited in by a depth-first traversal starting at the given vertex.
	// vector<vertex<T>> breadth_first(const int&) {vertex<T> v; return v;}; //Returns the vertices of the graph in the order they are visisted in by a breadth-first traversal starting at the given vertex.
	...
	// vector<vertex<T>> pre_order_traversal(const int&, directed_graph<T>&) {vector<vertex<T>>> v; return v;}; // returns the vertices in the visiting order of a pre-order traversal of the minimum spanning tree starting at the given vertex.
	// vector<vertex<T>> in_order_traversal(const int&, directed_graph<T>&) {vector<vertex<T>>> v; return v;}; // returns the vertices in the visiting order of an in-order traversal of the minimum spanning tree starting at the given vertex.
	// vector<vertex<T>> post_order_traversal(const int&, directed_graph<T>&) {vector<vertex<T>>> v; return v;}; // returns the vertices in ther visitig order of a post-order traversal of the minimum spanning tree starting at the given vertex.

	// vector<vertex<T>> significance_sorting(){vector<vertex<T>>> v; return v;}; // Return a vector containing a sorted list of the vertices in descending order of their significance.
...
template <typename T>
bool directed_graph<T>::contains(const int& u_id) const { return true; }
