...
bool directed_graph<T>::contains(const int& u_id) const {
	//for (vector<vertex<T>> v : this->adj_matrix) {
	//	if (v[0].id == u_id) return true;
	//}
...
bool directed_graph<T>::adjacent(const int& u_id, const int& v_id) const {
	...
	////cout << endl;
	////cout << "looking for " << u_id << " " << v_id << endl;
	//for (int i = 0; i < this->adj_matrix.size(); i++) {
	//	if (this->adj_matrix[i][0].id == u_id) {
	//		for (int j = 1; j < this->adj_matrix[i].size(); j++) {
	//			if (this->adj_matrix[i][j].id == v_id) return true;
	//		}
	//	}
	//}
	//if( this->adj_matrix.find() )
	return false;
}

			//cout << this->adj_matrix[i][0].id << " es " << u_id <<endl;
...
void directed_graph<T>::add_vertex(const vertex<T>& u) {
	...
	if(!contains(u.id)){
		...
		this->adj_matrix[u.id] = unordered_map<int, T>();
	}
}
...
void directed_graph<T>::remove_vertex(const int& u_id) {
	//for(int i = 0; i < this->adj_matrix.size(); i++){
	//	if (this->adj_matrix[i][0].id == u_id) {
	//		this->adj_matrix.erase(this->adj_matrix.begin() + i);
	//	}
	//}
}
...
size_t directed_graph<T>::num_vertices() const { return this->adj_matrix.size(); }
...
vector<vertex<T>> directed_graph<T>::get_vertices() {
	vector<vertex<T>> output_list;	// iterate and create list
	//for (vector<vertex<T>> v : this->adj_matrix) {
	//	output_list.push_back(v[0]);
	//}
	return output_list;
}
